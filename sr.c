...
/* ******************************************************************
   ...
   Network properties:
   - one way network delay averages five time units
...
#define SEQSPACE (2*WINDOWSIZE)    /* sequence space for SR */
#define NOTINUSE (-1)   /* unused header field value */
...
static struct pkt buffer[WINDOWSIZE]; /* buffer for packets waiting for ACK */
static int baseseqnum_a; /* first seq num of the window */
...
static struct pkt buffer_b[WINDOWSIZE];    /* buffer for received packets */
static int baseseqnum_b;        /* first seq num of receiver's window */
static int receivelast; /* last received packet position */
